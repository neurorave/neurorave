...
def timbral_depth(fname, fs=0, dev_output=False, phase_correction=False, clip_output=False, threshold_db=-60,
                  ...
                  db_decay_threshold=-40, mean=False):
    """
    ...
    if (mean):
        ...
        weighted_mean_normalised_lower_centroid = np.array(all_normalised_lower_centroid) * np.array(all_normalised_centroid_tpower)
    ...
    limited_weighted_mean_normalised_lower_centroid[limited_weighted_mean_normalised_lower_centroid > centroid_crossover_frequency] = np.float64(centroid_crossover_frequency)
    ...
    if (mean):
        ...
        weighted_mean_normalised_lower_ratio = np.array(all_normalised_lower_ratio) * (all_normalised_ratio_tpower)
    ...
    if dev_output:
        ...
        coefficients = np.array([-0.0043703565847874465, 32.83743202462131, 4.750862716905235, -14.217438690256062,
                                 3.8782339862813924, -0.8544826091735516, 66.69534393444391])[:, np.newaxis]
        ...
        metric3 = librosa.core.resample(metric3, orig_sr=metric3.shape[0], target_sr=metric2.shape[0]) 
