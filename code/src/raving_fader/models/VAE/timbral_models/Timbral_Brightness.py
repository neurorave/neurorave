...
def timbral_brightness(fname, fs=0, dev_output=False, clip_output=False, phase_correction=False, threshold=0,
                       ratio_crossover=2000, centroid_crossover=100, stepSize=1024, blockSize=2048, minFreq=20, mean=False):
    """
    ...
    ratio_highpass_audio *= (1.0 / (max(abs(audio_samples)) + 1e-9))
    centroid_highpass_audio *= (1.0 / (max(abs(audio_samples)) + 1e-9))
    audio_samples *= (1.0 / (max(abs(audio_samples)) + 1e-9))
    ...
    if dev_output:
        ...
        coefficients = np.array([4.613128018020465, 17.378889309312974, 17.434733750553022])[:, np.newaxis]
