...
def timbral_hardness(fname, fs=0, dev_output=False, phase_correction=False, clip_output=False, max_attack_time=0.1,
                     bandwidth_thresh_db=-75, mean=False):
    """
    ...
    if (mean):
        ...
        log_weighted_bandwidth_max[log_weighted_bandwidth_max <= 512] = fs / 512.0
    ...
    if dev_output:
        ...
        all_metrics[2, :] = np.repeat(log_weighted_midband_level, all_metrics.shape[0])
        ...
        coefficients = np.array([12.079781720638145, 18.52100377170042, 14.139883645260355, 5.567690321917516,
                                 3.9346817690405635, -4.326890461087848, -85.60352209068202])[:, np.newaxis]
