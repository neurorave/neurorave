...
def timbral_roughness(fname, dev_output=False, phase_correction=False, clip_output=False, fs=0, peak_picking_threshold=0.01, mean=False):
    """
    ...
    if (mean):
        ...
        mean_roughness = np.array(allroughness)
        mean_roughness[mean_roughness < 0.01] = 0.01
    ...
    if dev_output:
        ...
        #     return 0
        #else:
        roughness = np.log10(mean_roughness+1e-9) * 13.98779569 + 48.97606571545886
        if clip_output:
