...
def warm_region_cal(audio_samples, fs):
    ...
    for i in range(windowed_samples.shape[0]):
        ...
        N_entire, N_single = timbral_util.specific_loudness(samples,
                                                            Pref=100.0,
                                                            fs=fs,
                                                            Mod=0)
...
def timbral_warmth(fname,
    ...
    for idx, onset in enumerate(original_onsets):
        ...
        if segment_length < max_FFT_frame_size:
            freq, time, spec = spectrogram(segment,
                                           fs,
                                           nperseg=segment_length,
                                           nfft=max_FFT_frame_size)
        else:
            freq, time, spec = spectrogram(segment,
                                           fs,
                                           nperseg=max_FFT_frame_size,
                                           nfft=max_FFT_frame_size)
        ...
        peak_idx, peak_value, peak_x = timbral_util.detect_peaks(spec,
                                                                 freq=freq,
                                                                 fs=fs)
        # find lowest peak
        ...
        '''
        above_WR_spec = np.log10(spec[WR_upper_f_limit_idx:] + 0.01)
        above_WR_freq = np.log10(freq[WR_upper_f_limit_idx:] + 0.01)
    ...
    if (mean):
        ...
        mean_SC = np.log10(np.array(all_SC))
        mean_decay_score = np.array(all_decay_score)
        weighted_mean_ratio = np.array(all_ratio) * np.array(all_rms)
    ...
    if dev_output:
        ...
        '''
        all_metrics = np.ones((6, mean_SC.shape[0]))
        all_metrics[0, :] = mean_SC
        all_metrics[1, :] = np.repeat(weighted_hf, mean_SC.shape[0])
        all_metrics[2, :] = np.repeat(mean_wr, mean_SC.shape[0])
        all_metrics[3, :] = mean_decay_score
        all_metrics[4, :] = weighted_mean_ratio

        coefficients = np.array([
            -4.464258317026696, -0.08819320850778556, 0.29156539973575546,
            17.274733561081554, 8.403340066029507, 45.21212125085579
        ])[:, np.newaxis]

        warmth = np.sum(all_metrics * coefficients, axis=0)
