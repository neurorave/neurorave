...
def sharpness_Fastl(loudspec):
    ...
    gz = np.ones(140)
    z = np.arange(141, n + 1)
    gzz = 0.00012 * (z / 10.0)**4 - 0.0056 * (z / 10.0)**3 + 0.1 * (
        z / 10.0)**2 - 0.81 * (z / 10.0) + 3.5
...
def timbral_sharpness(fname,
    ...
    windowed_audio = timbral_util.window_audio(audio_samples,
                                               window_length=4096)
    ...
    if (mean):
        ...
        rms_sharpness = np.average(windowed_sharpness,
                                   weights=(windowed_rms * windowed_rms))
        # take the logarithm to better much subjective ratings
        rms_sharpness = np.log10(rms_sharpness)
    else:
        rms_sharpness = np.log10(np.array(windowed_sharpness) + 1)

    if dev_output:
        return [rms_sharpness]
    else:

        all_metrics = np.ones((2, rms_sharpness.shape[0]))
        all_metrics[0, :] = rms_sharpness

        # coefficients from linear regression
        coefficients = np.array([102.50508921364404,
                                 34.432655185001735])[:, np.newaxis]

        # apply regression
        sharpness = np.sum(all_metrics * coefficients, axis=0)
