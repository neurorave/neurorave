...
def timbral_depth(fname,
                  ...
                  centroid_crossover_frequency=2000,
                  ratio_crossover_frequency=500,
    ...
    lowpass_centroid_audio_samples = timbral_util.filter_audio_lowpass(
        ...
        crossover=centroid_crossover_frequency,
        fs=fs)
    lowpass_centroid_audio_samples = timbral_util.filter_audio_lowpass(
        lowpass_centroid_audio_samples,
        crossover=centroid_crossover_frequency,
    ...
    lowpass_ratio_audio_samples = timbral_util.filter_audio_lowpass(
        lowpass_ratio_audio_samples,
        crossover=ratio_crossover_frequency,
        fs=fs)
    lowpass_ratio_audio_samples = timbral_util.filter_audio_lowpass(
        lowpass_ratio_audio_samples,
        crossover=ratio_crossover_frequency,
    ...
    if len(audio_samples) > nfft:
        freq, time, spec = spectrogram(audio_samples, fs, 'hamming', nfft,
                                       hop_size, nfft, 'constant', True,
                                       'spectrum')
        lp_centroid_freq, lp_centroid_time, lp_centroid_spec = spectrogram(
            lowpass_centroid_audio_samples, fs, 'hamming', nfft, hop_size,
            nfft, 'constant', True, 'spectrum')
        lp_ratio_freq, lp_ratio_time, lp_ratio_spec = spectrogram(
            lowpass_ratio_audio_samples, fs, 'hamming', nfft, hop_size, nfft,
            'constant', True, 'spectrum')

    else:
        # file is shorter than 4096, just take the fft
        freq, time, spec = spectrogram(audio_samples, fs, 'hamming',
                                       len(audio_samples),
                                       len(audio_samples) - 1, nfft,
                                       'constant', True, 'spectrum')
        lp_centroid_freq, lp_centroid_time, lp_centroid_spec = spectrogram(
            lowpass_centroid_audio_samples, fs, 'hamming',
            len(lowpass_centroid_audio_samples),
            len(lowpass_centroid_audio_samples) - 1, nfft, 'constant', True,
            'spectrum')
        lp_ratio_freq, lp_ratio_time, lp_ratio_spec = spectrogram(
            lowpass_ratio_audio_samples, fs, 'hamming',
            len(lowpass_ratio_audio_samples),
            len(lowpass_ratio_audio_samples) - 1, nfft, 'constant', True,
            'spectrum')
    ...
    if (mean):
        ...
        weighted_mean_normalised_lower_centroid = np.array(
            all_normalised_lower_centroid) * np.array(
                all_normalised_centroid_tpower)
    ...
    limited_weighted_mean_normalised_lower_centroid[
        limited_weighted_mean_normalised_lower_centroid >
        centroid_crossover_frequency] = np.float64(
    ...
    if (mean):
        ...
        weighted_mean_normalised_lower_ratio = np.array(
    ...
    rms_step_size = 256
    rms_envelope = timbral_util.calculate_rms_enveope(audio_samples,
                                                      step_size=rms_step_size)
    ...
    for idx, onset in enumerate(onsets):
        ...
        if min(post_max_segment) >= decay_threshold:
            ...
            my_duration = np.where(
                post_max_segment < decay_threshold)[0][0] / time_convert
    ...
    if (mean):
        ...
        mean_my_duration = np.log10(np.array(all_my_duration) + 1)
    '''
    ...
    # normalise this
    norm_spec = (all_spectrum - np.min(all_spectrum)) / (np.max(all_spectrum) -
                                                         np.min(all_spectrum))
    # set limit for peak picking
    cthr = 0.01
    # detect peaks
    peak_idx, peak_value, peak_freq = timbral_util.detect_peaks(
        norm_spec, cthr=cthr, unprocessed_array=norm_spec, freq=freq)
    # estimate peak
    ...
    if dev_output:
        ...
        coefficients = np.array([
            -0.0043703565847874465, 32.83743202462131, 4.750862716905235,
            -14.217438690256062, 3.8782339862813924, -0.8544826091735516,
            66.69534393444391
        ])[:, np.newaxis]
        ...
        metric3 = librosa.core.resample(metric3,
                                        orig_sr=metric3.shape[0],
                                        target_sr=metric2.shape[0])
        metric4 = pitch_estimate
        metric5 = metric2 * metric3
        metric6 = timbral_util.sigmoid(metric2) * metric3

        # pack metrics into a matrix
        all_metrics = np.ones((7, metric1.shape[0]))
        ...
        depth = np.sum(all_metrics * coefficients, axis=0)
