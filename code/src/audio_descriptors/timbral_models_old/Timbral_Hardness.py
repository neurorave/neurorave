...
def timbral_hardness(fname,
    ...
    midband_level, weighed_midband_level = timbral_util.weighted_bark_level(
        audio_samples, fs, low_bark_band=70, upper_bark_band=140)
    ...
    '''
    HP_ratio = timbral_util.get_percussive_audio(audio_samples,
                                                 return_ratio=True)
    ...
    audio_samples = np.lib.pad(audio_samples, (nperseg + 1, 0),
                               'constant',
    ...
    original_onsets = timbral_util.calculate_onsets(audio_samples,
                                                    envelope,
                                                    fs,
                                                    nperseg=nperseg)
    ...
    mag = timbral_util.db2mag(
        bandwidth_thresh_db)  # calculate threshold in linear from dB
    bandwidth, t, f = timbral_util.get_bandwidth_array(
        audio_samples,
        fs,
        nperseg=nperseg,
        overlap_step=bandwidth_step_size,
        rolloff_thresh=mag,
        normalisation_method='none')
    # bandwidth sample rate
    bandwidth_fs = fs / float(
        bandwidth_step_size)  # fs due to spectrogram step size
    '''
    ...
    '''
    bandwidth_onset = np.array(onsets / float(bandwidth_step_size)).astype(
        'int')  # overlap_step=128
    '''
    ...
    for onset_count in range(len(bandwidth_onset)):
        ...
        if max(bandwidth_seg) > 0:
            ...
            if bandwidth_attack:
                ...
                if max_attack_time > 0:
                    max_attack_time_samples = int(max_attack_time *
                                                  bandwidth_fs)
                    if len(hold_bandwidth_seg[start_idx:]
                           ) > start_idx + max_attack_time_samples:
                        all_bandwidth_max.append(
                            max(hold_bandwidth_seg[start_idx:start_idx +
                                                   max_attack_time_samples]))
                    else:
                        all_bandwidth_max.append(
        ...
        if onset == original_onsets[-1]:
            ...
            attack_seg = np.array(envelope[onset:original_onsets[onset_count +
                                                                 1]])
            strength_seg = np.array(
                onset_strength[int(onset /
                                   512):int(original_onsets[onset_count + 1] /
                                            512)])
            audio_seg = np.array(
        ...
        centroid_int_samples = int(
            0.125 * fs)  # number of samples for attack time integration
    ...
    if (mean):
        ...
        log_weighted_bandwidth_max = np.array(all_bandwidth_max)
        log_weighted_bandwidth_max[
            log_weighted_bandwidth_max <= 512] = fs / 512.0
        log_weighted_bandwidth_max = np.log10(log_weighted_bandwidth_max + 1)
        log_attack_centroid = np.array(all_attack_centroid)
        log_attack_centroid[log_attack_centroid <= 200.0] = 200.0
        log_attack_centroid = np.log10(all_attack_centroid)
        mean_attack_time = np.array(all_attack_time)
        log_mean_max_strength = np.array(all_max_strength)
    '''
    ...
    if dev_output:
        ...
        '''
        all_metrics = np.ones((7, log_weighted_bandwidth_max.shape[0]))
        all_metrics[0, :] = log_weighted_bandwidth_max
        all_metrics[1, :] = log_attack_centroid
        all_metrics[2, :] = np.repeat(log_weighted_midband_level,
                                      all_metrics.shape[0])
        all_metrics[3, :] = np.repeat(log_HP_ratio, all_metrics.shape[0])
        ...
        coefficients = np.array([
            12.079781720638145, 18.52100377170042, 14.139883645260355,
            5.567690321917516, 3.9346817690405635, -4.326890461087848,
            -85.60352209068202
        ])[:, np.newaxis]

        hardness = np.sum(all_metrics * coefficients, axis=0)
