...
def timbral_extractor(fname,
    ...
    hardness = timbral_hardness(audio_samples,
                                fs=fs,
    ...
    depth = timbral_depth(audio_samples,
                          fs=fs,
                          dev_output=dev_output,
                          phase_correction=phase_correction,
                          clip_output=clip_output)
    ...
    brightness = timbral_brightness(audio_samples,
                                    fs=fs,
                                    dev_output=dev_output,
                                    phase_correction=phase_correction,
                                    clip_output=clip_output)
    ...
    roughness = timbral_roughness(audio_samples,
                                  fs=fs,
                                  dev_output=dev_output,
                                  phase_correction=phase_correction,
                                  clip_output=clip_output)
    ...
    warmth = timbral_warmth(audio_samples,
                            fs=fs,
                            dev_output=dev_output,
                            phase_correction=phase_correction,
                            clip_output=clip_output)
    ...
    sharpness = timbral_sharpness(audio_samples,
                                  fs=fs,
                                  dev_output=dev_output,
                                  phase_correction=phase_correction,
                                  clip_output=clip_output)
    ...
    boominess = timbral_booming(audio_samples,
                                fs=fs,
                                dev_output=dev_output,
                                phase_correction=phase_correction,
                                clip_output=clip_output)
    ...
    if output_type == 'dictionary':
        ...
        timbre = [
            hardness, depth, brightness, roughness, warmth, sharpness,
            boominess, reverb
        ]
    else:
        raise ValueError('output_type must be \'dictionary\' or \'list\'.')
