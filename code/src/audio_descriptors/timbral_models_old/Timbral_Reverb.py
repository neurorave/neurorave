...
def timbral_reverb(fname,
    ...
    raw_audio_samples, fs = timbral_util.file_read(fname,
                                                   fs=fs,
                                                   phase_correction=False,
                                                   mono_sum=False,
                                                   loudnorm=False)
...
def estimate_RT60(audio_samples, fs):
    ...
    par = init_rt_estimate_e(
        fs)  # struct with all parameters and buffers for frame-wise processing
    ...
    if BL < Laudio:
        ...
        for n in n_array:
            ...
            RT, par, finalrt = rt_estimate_frame_my(
                audio_samples[ind[np.arange(0, len(ind), par['down'])]], par)
...
def init_rt_estimate_e(fs=24000):
    ...
    par = {"fs": fs}
    no = par[
        'fs'] / 24000.0  # correction factor to account for different sampling frequency
    ...
    par['N_sub'] = int(round(
        no * 700 / par['down']))  # sub-frame length(after downsampling)
    par['N_shift'] = int(round(
        no * 200 / par['down']))  # frame shift(before downsampling)
    ...
    par['N'] = int(par['nos_max'] *
                   par['N_sub'])  # maximal frame length(after downsampling)
    ...
    par['Tquant'] = np.arange(
        Tmin, Tmax + par['bin'] / 2,
        par['bin'])  # set of qunatized RTs considered for maximum search
    par['a'] = np.exp(
        -3.0 * np.log(10) /
        (par['Tquant'] *
         (par['fs'] / par['down'])))  # corresponding decay rate factors
    par['La'] = len(
        par['a'])  # num of considered decay rate factors( = no of.RTs)
    ...
    par['buffer'] = np.zeros(
        par['buffer_size'])  # buffer with previous indices to update histogram
    par['no_bins'] = int(par['La'])  # no. of histogram bins
    par['hist_limits'] = np.arange(Tmin - par['bin'] / 2.0, Tmax + par['bin'],
                                   par['bin'])  # limits of histogram bins
    ...
    par['RT_raw'] = par[
        'RT_initial']  # raw RT estimate obtained by histogram - approach
...
def rt_estimate_frame_my(frame, par):
    ...
    for k in range(2, par['nos_max']):
        ...
        if (var_pre > var_cur) and (max_pre > max_cur) and (min_pre < min_cur):
            ...
            if cnt >= par['nos_min']:
                ...
                RTml, _ = max_loglf(frame[:cnt * par['N_sub']], par['a'],
                                    par['Tquant'])
        ...
        if k == par['nos_max']:
            ...
            RTml, _ = max_loglf(frame[0:cnt * par['N_sub']], par['a'],
                                par['Tquant'])
    ...
    if RTml >= 0:  # new ML estimate calculated
        ...
        for i in range(par['no_bins']):
            ...
            # find index corresponding to the ML estimate
            if (RTml >= par['hist_limits'][i]) and (RTml <=
                                                    par['hist_limits'][i + 1]):
        ...
        if par['hist_counter'] > par['buffer_size'] + 1:
            ...
            par['hist_rt'][int(
                par['buffer'][0])] = par['hist_rt'][int(par['buffer'][0])] - 1

        par['buffer'] = np.append(par['buffer'][1:],
                                  index)  # % update buffer with indices
        idx = np.argmax(
            par['hist_rt'])  # find index for maximum of the histogram
...
def max_loglf(h, a, Tquant):
    ...
    for i in range(len(a)):
        ...
        sigma = (1 / N) * sum1
        ll[i] = -N * np.log(2) - N * np.log(sigma) - np.sum(np.log(
            a[i]**n)) - (1 / sigma) * sum1
...
def reverb_logistic_regression(mean_RT60):
    ...
    logit_model = np.sum(
        np.array(coefficients) * np.array(attributes)) + intercept
