...
class DDSP(nn.Module):
    ...
    def __init__(self, n_latent, hidden_size, n_harmonic, n_bands,
                 sampling_rate, block_size,n_descriptors=0):
        super().__init__()
        self.register_buffer("sampling_rate", torch.tensor(sampling_rate))
        self.register_buffer("block_size", torch.tensor(block_size))

        self.encoder = Encoder(n_latent, n_mfcc=30, n_hidden=hidden_size)

        self.in_mlps = nn.ModuleList([mlp(1, hidden_size, 3)] * 2 +
                                     [mlp(n_latent+n_descriptors, hidden_size, 3)])
    ...
    def forward(self, mfcc, pitch, loudness):
        z = self.encoder(mfcc)
        hidden = torch.cat([
            self.in_mlps[0](pitch), self.in_mlps[1](loudness),
            self.in_mlps[2](z)
        ], -1)
    ...
    def decode(self, z, pitch, loudness):
        hidden = torch.cat([
            self.in_mlps[0](pitch), self.in_mlps[1](loudness),
            self.in_mlps[2](z)
        ], -1)
